...
import ch.zhaw.catan.game.SiedlerGame;
...
public class ThreePlayerStandard {
    ...
    public static final Map<Integer, Map<Config.Faction, Map<Resource, Integer>>> INITIAL_DICE_THROW_PAYOUT = Map.of(
            2, Map.of(
                    Config.Faction.values()[0], Map.of(Resource.GRAIN, 1)),
            3, Map.of(
                    Config.Faction.values()[1], Map.of(Resource.WOOL, 1)),
            4, Map.of(
                    Config.Faction.values()[0], Map.of(Resource.ORE, 1),
                    Config.Faction.values()[2], Map.of(Resource.BRICK, 1)),
            5, Map.of(
                    Config.Faction.values()[2], Map.of(Resource.LUMBER, 1)),
            6, Map.of(
                    Config.Faction.values()[0], Map.of(Resource.LUMBER, 1)),
            8, Map.of(
                    Config.Faction.values()[1], Map.of(Resource.WOOL, 1)),
            9, Map.of(
                    Config.Faction.values()[1], Map.of(Resource.GRAIN, 1)),
            10, Map.of(),
            11, Map.of(
                    Config.Faction.values()[0], Map.of(Resource.BRICK, 1)),
            12, Map.of(
                    Config.Faction.values()[0], Map.of(Resource.WOOL, 1),
                    Config.Faction.values()[1], Map.of(Resource.WOOL, 1))
    );
    ...
    public static SiedlerGame getAfterSetupPhase(int winpoints) {
        ...
        for (int i = 0; i < model.getPlayerFactions().size(); i++) {
            ...
            Assertions.assertTrue(model.placeInitialSettlement(INITIAL_SETTLEMENT_POSITIONS.get(f).first, false));
            Assertions.assertTrue(model.placeInitialRoad(INITIAL_SETTLEMENT_POSITIONS.get(f).first, INITIAL_ROAD_ENDPOINTS.get(f).first));
            model.switchToNextPlayer();
        }
        for (int i = 0; i < model.getPlayerFactions().size(); i++) {
            model.switchToPreviousPlayer();
            Config.Faction f = model.getCurrentPlayerFaction();
            Assertions.assertTrue(model.placeInitialSettlement(INITIAL_SETTLEMENT_POSITIONS.get(f).second, true));
            Assertions.assertTrue(model.placeInitialRoad(INITIAL_SETTLEMENT_POSITIONS.get(f).second, INITIAL_ROAD_ENDPOINTS.get(f).second));
        }
        return model;
    }
    ...
    private static void buildSettlement(SiedlerGame model, Point position, List<Point> roads) {
        ...
        Assertions.assertTrue(model.buildSettlement(position));
    }

    private static void buildRoad(SiedlerGame model, List<Point> roads) {
        for (int i = 0; i < roads.size() - 1; i++) {
            Assertions.assertTrue(model.buildRoad(roads.get(i), roads.get(i + 1)));
        }
    }
}
